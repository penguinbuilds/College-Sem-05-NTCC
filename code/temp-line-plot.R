...
# from the normalized temperatures of 1951 to 1980 as a heatmap.
# The lines are colored according to the year.
...
t_data %>%
    ggplot(aes(
        x = month_number, y = t_diff, group = year,
        color = year, linewidth = this_year
    )) +
    ...
    theme(
        panel.background = element_rect(
            fill = "black", color = "white", linewidth = 1
        ),
...
ggsave("../figures/temp-line-plot.png", width = 8, height = 4.5)
